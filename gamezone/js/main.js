$(() => {
                ...
                success:(data, textStatus, xhr) => {
                    if(xhr.status === 200){
                        ...
                        $.each(data, (i, val) => {
                            ...
                            const dlLink = category === "HTML5" ? `<button class="playBtn">PLAY</button>` : `<a href="${this.contenDir}/${originCatName.toLowerCase()}/${subCategory.toLowerCase()}/${contentName.toLowerCase()}/${modFileName}">DOWNLOAD</a>`
                    ...
                    if(xhr.status === 200){
                        $.each(data, (iData, dataVal) => {
                            ...
                            $.each($('.mainCategory'), (iMainCategory, mainCat) => { 
                                if(main === $(mainCat).attr('data-namecat')){
    ...
    class Content extends Utils{
        ...
        showHtmlGame(data, catName, subCatName, originCatName){
            ...
            this.siteContainer.append(`
                <div class="contentHtmlGame">
                    <div class="closeHtmlGame">
                        <span></span>
                        <span></span>
                    </div>
                    <div class="htmlGameWrapper">
                        <iframe gesture="media" allow="encrypted-media" allowfullscreen src="${filename}" title="${title}"></iframe>
                    </div>
                </div>
            `)

            $('.closeHtmlGame').on('click', (e) => {
                gsap.to($(e.currentTarget).parent(), {opacity:0, duration:.3, ease:"linear", onComplete:() => {
                    ...
                    if($('.contentPreview').length > 0){
                        ...
                        gsap.fromTo($('.contentPreview'), {opacity:0}, {opacity:1, duration:.3, ease:"linear"})
                    }else{
        ...
        showPreview(data, catName, subCatName, originCatName){
            const { contentId, title, icon, description, filename, screenshots } = data
            ...
            switch(catName){
                case "HTML5":
                        thumbnail = `<img src="${this.contenDir}/${originCatName.toLowerCase()}/${subCatName.toLowerCase()}/${contentName.toLowerCase()}/${icon}" alt="${title}" />`

                        dlLink = `<button class="playBtn">PLAY</button>`
                ...
                case "Games":
                    ...
                    dlLink = `<a href="${this.contenDir}/${originCatName.toLowerCase()}/${subCatName.toLowerCase()}/${contentName.toLowerCase()}/${modFileName}">DOWNLOAD</a>`
        ...
        clickPlayBtn(){
            $('.playBtn').on('click', async (e) => {
                this.showLoader();

                const contentId = $(e.currentTarget).parent().prev().prev().val()
                const spanText = $(e.currentTarget).prev().text()
                const catName = spanText.slice(0, spanText.indexOf(" "));
                const subCatName = spanText.slice((spanText.indexOf(" ") + 3), spanText.length)
                const originCatName = $(e.currentTarget).parent().prev().prev().prev().val()
                ...
                if(res.length != 0){
                    ...
                    this.showHtmlGame(res[0], catName, subCatName, originCatName)
                    this.removeLoader();
                }
